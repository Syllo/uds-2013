...
static inline void mult2(int i, DATA_TYPE POLYBENCH_2D(path,N,N,n,n), int k){
    if(i!=k)
        path[i][k] = path[i][k] < path[i][k] + path[k][k] ? path[i][k] : path[i][k] + path[k][k];
    path[k][i] = path[k][i] < path[k][i] + path[k][k] ? path[k][i] : path[k][i] + path[k][k];
}
static inline void mult3(int i, DATA_TYPE POLYBENCH_2D(path,N,N,n,n), int n, int k){
    for(int j=k+1; j< n; j++){
        path[i][j] = path[i][j] < path[i][k] + path[k][j] ? path[i][j] : path[i][k] + path[k][j];
    }
}
static inline void mult4(int j, DATA_TYPE POLYBENCH_2D(path,N,N,n,n), int n, int k){
    for(int i=k+1; i< n; i++){
        path[i][j] = path[i][j] < path[i][k] + path[k][j] ? path[i][j] : path[i][k] + path[k][j];
    }
}
...
#pragma scop
    ...
    for(int k=0; k< _PB_N; k++){
        multifor(i1=0,i2=0,i3=0,j4=0; i1<k, i2<k+1, i3<k+1, j4<k+1; i1++,i2++,i3++,j4++; 1,1,1,1; 0,1,2,2){
0:{
      mult1(i1, path, k);
  }
1:{
